...
const ARTIFACT_DIR = path.resolve("artifacts/vercel-onboarding");
...
async function run() {
  ...
  try {
    await page.goto("https://vercel.com/signup", { waitUntil: "domcontentloaded" });
    await page.screenshot({ path: path.join(ARTIFACT_DIR, "01-signup-page.png"), fullPage: true });
    report.steps.push({ name: "open-signup", status: "ok", at: nowIso() });

    await waitForOperator(
      "Complete Vercel signup/login (email OTP/CAPTCHA if prompted) in the opened browser.",
    );
    ...
    await context.storageState({
      path: path.join(ARTIFACT_DIR, "vercel-storage-state.json"),
    });
